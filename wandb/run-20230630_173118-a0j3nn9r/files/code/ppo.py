...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, action=None):
        ...
        if action is None:
            action = probs.sample()
        return action, probs.log_prob(action), self.critic(x.clone())

    def forward(self, x):
        value_estimates = self.critic(x.clone())
        action_logits = self.actor(x.clone())
...
def update(agent, optimizer, states, actions, log_probs, returns, advantages, epsilon_clip, policy_epochs,step):
    ...
    for _ in range(policy_epochs):
        ...
        for start in range(0, step, minibatch_size):
            ...
            ratio = torch.exp(action_log_probs.item() - log_prob.item())
...
def ppo(env, num_episodes, gamma, epsilon_clip, policy_epochs, learning_rate,episode_rewards):
    ...
    while episode < num_episodes:
        ...
        for i in range(step):
            _, value_estimates = agent.forward(states[i])
