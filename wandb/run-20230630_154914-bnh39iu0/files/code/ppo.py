...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, action=None):
        if isinstance(x,list):
            log_prob = torch.zeros(len(x))

            value = torch.zeros(len(x))
            ...
            for i in range(len(x)):
                ...
                action = probs.sample()
                v = self.critic(x[i])
                log_prob[i] = probs.log_prob(action)
                value[i] = v
            return None,log_prob,value
...
def update(agent, optimizer, states, actions, log_probs, returns, advantages, epsilon_clip, policy_epochs):
    ...
    # print(b_inds)
    num_minibatches = 4
    minibatch_size = len(states) // num_minibatches
    for _ in range(48):
        ...
        # for i in range(len(states)):
            ...
            ratio = (action_log_probabilities - torch.tensor(log_prob)).exp()
...
if __name__ == "__main__":
    ...
    ppo(env, 128, 0.99, 0.2, 4, 2.5e-4, episode_rewards)
