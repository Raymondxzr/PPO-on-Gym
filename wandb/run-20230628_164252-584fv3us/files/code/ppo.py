...
def update(agent, optimizer, states, actions, log_probs, returns, advantages, epsilon_clip, policy_epochs, value_epochs):
    """
    ...
    for _ in range(policy_epochs):
        for i in range(len(states)):
            ...
            advantage = advantages[state][action]
            # print(advantage)
...
def ppo(env, num_episodes, batch_size, gamma, epsilon_clip, policy_epochs, value_epochs, learning_rate,episode_rewards):
    """
    ...
    while episode < num_episodes:
        ...
        for reward in reversed(rewards):
            ...
            # discounted_reward = discounted_reward.clone()
            returns[states[idx]] = discounted_reward
            idx -= 1
        idx = 0
        for state in states:
            # state = state.clone()
            if state not in advantages:
                # advantages[state] = torch.zeros(env.action_space.n)
                advantages[state] = [torch.tensor(0)] * env.action_space.n
                # print(advantages[state])
            # cloned_state = state.clone()
            value_estimates = agent.critic(state)
            # print(actions[idx].item())

            advantages[state][actions[idx].item()] = returns[state] - value_estimates
            idx += 1
        update(agent, optimizer, states, actions, log_probs, returns, advantages, epsilon_clip, policy_epochs, value_epochs)
        episode+=1
...
if __name__ == "__main__":
    ...
    ppo(env, 20, 0, 0.99, 0.2, 4, 4, 2.5e-4, episode_rewards)
